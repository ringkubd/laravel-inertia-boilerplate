                ...
                <div class="card-body" id="printme">
                    ...
                    <div class="flex flex-col justify-center justify-content-center" style="flex: auto; justify-content: center; align-items: center; width: 100%">
                        <table class="table table-auto bank_sheet" style="border: 1px solid rgb(0,0,0)!important; color: black!important;width: 100%">
                            ...
                            <tbody>
                                <template v-for="(invoice, index) in data.filter(i => i.amount > 0)" :key="invoice.id">
                            <tr v-for="(invoice, index) in data.filter(i => i.amount > 0)" :key="invoice.id" :class="'border-1' +(index % 25 === 0 ? ' table-page-break': ' table-row') " :style="(index % 25 === 0 ? ' margin-top: 1.7in !important': ' table-row')">
                                <td class="text-center">{{ index + 1 }}</td>
