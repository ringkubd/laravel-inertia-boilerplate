...
class InvoiceController extends Controller
{
    ...
    public function show($invoice_id): Response
    {
        ...
        $lastSemester = $basicInfo->semester - 1;
        $numberOfMmaTillLastSemester = Invoice::where('session', $basicInfo->session)
            ->where('semester', '<=', $lastSemester)
            ->whereRaw(DB::raw('fee_type like "%MMA%"'))
            ->groupBy('invoice_no', 'semester')
            ->get()
            ->count();
        ...
        return Inertia::render('Invoice/Invoice', [
            ...
            'basicInfo' => $basicInfo,
            'last_mma' => $lastMma,
            'number_of_mma_till_last_semester' => $numberOfMmaTillLastSemester
        ]);
    }
